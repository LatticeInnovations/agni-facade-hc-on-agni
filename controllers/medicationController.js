...
const {fetchResource, getTransformedResult, handleError, buildFHIRResource} = require("../services/helperFunctions");
...
let updateMedicationData = async function (req, res) {    
    try {
        ...
        for (let medicine of req.body) {
            ...
            const idsToDelete = existingMedKnowledge.filter(cond => removed.includes(cond.resource.code?.coding?.[0]?.code))
...
const setMedicineSaveResponse  = (reqBundleData, responseBundleData, type) => {
    ...
    filteredData = responseData.filter(e => (e.resource.resourceType == "Medication" || e.resource.resourceType == "Organization" )|| (type == "patch" && e.resource.resourceType == "Binary"));
