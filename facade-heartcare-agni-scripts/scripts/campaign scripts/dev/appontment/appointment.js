/**
 * FHIR Appointment - Add serviceType (Facility) Updater
 *
 * Usage:
 *   node update-Appointment-service-type.js --env dev --token <your_token>
 *   node update-Appointment-service-type.js --env prod --token <your_token>
 *   node update-Appointment-service-type.js --env dev --token <your_token> --dry-run
 *
 * Alternatively, set the token via environment variable (recommended for prod):
 *   FHIR_TOKEN=<your_token> node update-Appointment-service-type.js --env prod
 *
 * Requirements:
 *   - Node.js 18+
 *   - A Appointment.json file in the same directory (FHIR Bundle or array of Appointment resources)
 *   - Appointment?service-type:missing=true&_count=1000
 */
...
const AppointmentS_FILE = path.join(__dirname, "appointment.json");
...
function loadAppointments(filePath) {
  if (!fs.existsSync(filePath)) {
    console.error(`Appointment.json not found at: ${filePath}`);
    process.exit(1);
  }
  ...
  if (raw.resourceType === "Appointment") return [raw];

  console.error("Appointment.json must be a FHIR Bundle, array of Appointments, or a single Appointment resource.");
  process.exit(1);
}

// ─── Appointment HELPERS ─────────────────────────────────────────────────────────

function alreadyHasServiceType(Appointment) {
  return !!Appointment.serviceType;
}

function addServiceType(Appointment) {
  return { ...Appointment, serviceType: SERVICE_TYPE_TO_ADD };
}
...
async function putAppointment(baseUrl, token, Appointment) {
  const url  = `${baseUrl}/Appointment/${Appointment.id}`;
  ...
  if (res.status === 403) {
    throw new Error(`Forbidden (403) — token does not have permission to update Appointments.`);
  }
...
async function main() {
  ...
  console.log(`\n🏥  FHIR Appointment serviceType Updater`);
  console.log(`   Environment : ${env}`);
  console.log(`   Server URL  : ${baseUrl}`);
  console.log(`   Dry run     : ${dryRun}`);
  console.log(`   Token       : ${token.slice(0, 6)}${"*".repeat(10)} (truncated for safety)`);
  console.log(`   Source file : ${AppointmentS_FILE}\n`);

  const Appointments = loadAppointments(AppointmentS_FILE);
  console.log(`📋  Loaded ${Appointments.length} Appointment(s) from file.\n`);

  const results = { skipped: 0, updated: 0, failed: 0 };

  for (const Appointment of Appointments) {
    const id = Appointment.id ?? "(no id)";

    if (!Appointment.id) {
      console.warn(`  ⚠️  Appointment without id — skipped.`);
      results.skipped++;
      continue;
    }

    if (alreadyHasServiceType(Appointment)) {
      console.log(`  ⏭️  Appointment/${id} — serviceType already present, skipping.`);
      results.skipped++;
      continue;
    }

    const updated = addServiceType(Appointment);

    if (dryRun) {
      console.log(`  🔍  [DRY RUN] Would PUT Appointment/${id}`);
    ...
    try {
      await putAppointment(baseUrl, token, updated);
      console.log(`  ✅  Appointment/${id} — updated successfully.`);
      results.updated++;
    } catch (err) {
      console.error(`  ❌  Appointment/${id} — FAILED: ${err.message}`);
      results.failed++;
    }
  }
